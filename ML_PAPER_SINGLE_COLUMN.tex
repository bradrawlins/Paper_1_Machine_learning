...
J=\frac{1}{n}\sum^n_{i=1}(y_i-\hat{y_i})^2
\end{equation} 
...
\beta_{m} = \beta_m-\gamma\frac{\partial}{\partial\beta_m}J(\beta_m)
\end{equation}
where $\gamma$ is known as the learning rate.\\
...
The result of the equation \ref{eqn_summed_sig} is subsequently passed to an activation function ($\overline{h}_l = \sigma_l(\overline{z}_l)$). There are various activation functions that can be utilized, such as linear, ReLu, Elu, and the hyperbolic-tangent \citep{goodfellow}, with the final layer activation function usually being linear for regression models. The current work makes use of RelU and linear activation functions. Equation \ref{eqn_act_func} highlights the ReLu and linear activation functions.
