...
MDNs are used to predict the parameters of a probability distribution ($P(\bar{X}\mid\bar{Y})$) allowing for non-Gaussian distributions to be modelled, thus making MDNs a probabilistic machine learning framework. MDNs estimate the conditional probability distribution as a mixture of Gaussian distributions where the mixing coefficients ($\bar{\pi}_k$) and component densities are flexible functions of the input data ($\bar{X}$). Equation \ref{eqn_cond_prob_func} illustrates the conditional probability function.

\begin{equation}\label{eqn_cond_prob_func}
P(\bar{X}\mid\bar{Y}) = \sum_{k=1}^K\pi_k(\bar{X})\cdot \mathbb{N}(\bar{Y}\mid\bar{\mu}_k(\bar{X}),\bar{\sigma}^2_k(\bar{X}))
\end{equation}
where $K$ represents the number of selected normal distributions, while $\bar{\mu}_k$ and $\bar{\sigma}^2_k$ are the predicted means and variances for each distribution $k$ given the input data $\bar{X}$ respectively.\\

A schematic of a simple ANN-MDN network is given in Figure \ref{fig_mdn_schematic}, highlighting the mixing coefficients, predicted means and deviations. It can is shown that modifications are made to the output layer by splitting the network output into three parts in order to calculate the $\bar{\pi}_k$, $\bar{\mu}_k$ and $\bar{\sigma}^2_k$ for each $k$ distribution. This enables the MDN network to learn the $P(\bar{X}\mid\bar{Y})$.\\
...
\begin{split}
&\sum_{k=1}^K\bar{\pi}_k^n=1\\
&0\leq\bar{\pi}_k^n\leq1
\end{split}
\end{equation}
...
\bar{\pi}_k^n(\bar{x}^n)=\frac{exp(h_k^{\pi,n})}{\sum_{k=1}^Kexp(h_k^{\pi,n})}
\end{equation}
...
(\bar{\sigma}^n_k(\bar{x}^n))^2\geq0
\sigma_k^n(\bar{x}^n)=exp(h_k^{\sigma,n})
\end{equation}
...
J_{NLL}(\bar{Y},\bar{\pi},\bar{\sigma},\bar{\mu})=-\sum^N_{n=1}ln\left\lbrace\sum^K_{k=1}\bar{\pi}_k(\bar{X}^n,\bar{\theta})\cdot\mathbb{N}(\bar{Y}^n\mid\bar{\mu}_k(\bar{X}^n,\bar{\theta}),\bar{\sigma}^2_k(\bar{X}^n,\bar{\theta})) \right\rbrace
\end{equation}
