...
\title [mode = title]{Validation of an integrated data-driven surrogate model and a thermo-hydraulic network based model to determine thermal response of a 620 $MWe$ utility scale boiler using a fully connected mixture density network}  
...
\toprule
 Parameter& MLP search space & ANN-MDN search space \\ % Table header row
\midrule
...
As concluded in Section \ref{sec_hyper}, the ANN-MDN model is the model used for the surrogate model integration. The subsequent sections provide a validation of the integrated models thermal response and a case study the investigates the effects of using a poor quality fuel. 

\subsection{Multiple load validation}
The surrogate models thermal prediction and response are validated for 100\%, 80\% and 60\% MCR load cases. The measured plant data for the respective load ratings were made available, allowing foe the mean and standard deviation to be calculated from the plant data, thus capturing the uncertainty of the measurement devices used in the plant. 

use of  amonte-carlo simulation ruin to determine the confidence band for the 

Figure \ref{} (a-c)

What does the thermal response imply
discuss the results. The use of MDn provides a confidence band.
\begin{figure*}\label{fig_100_valid}
\centering
...
\begin{subfigure}{0.33\textwidth}
    ...
    \caption{}
\end{subfigure}
\caption{100\% MCR load validation result comparison of the measured data and ANN-MDN model results for (a) the various heat loads, (b) the steam exit and generation flowrates and (c) the required attemperator flowrates to maintain operational integrity}
\end{figure*}

\begin{figure*}\label{fig_80_valid}
\centering
\begin{subfigure}{0.33\textwidth}
...
\begin{subfigure}{0.33\textwidth}
    ...
    \caption{}
\end{subfigure}
\caption{80\% MCR load validation result comparison of the measured data and ANN-MDN model results for (a) the various heat loads, (b) the steam exit and generation flowrates and (c) the required attemperator flowrates to maintain operational integrity}
\end{figure*}

\begin{figure*}\label{fig_60_valid}
\centering
\begin{subfigure}{0.33\textwidth}
...
\begin{subfigure}{0.33\textwidth}
    ...
    \caption{}
\end{subfigure}
\caption{60\% MCR load validation result comparison of the measured data and ANN-MDN model results for (a) the various heat loads, (b) the steam exit and generation flowrates and (c) the required attemperator flowrates to maintain operational integrity}
\end{figure*}

\subsection{Utility boiler response for a fuel with high ash \& moisture contents}
Mention the importance of fuel energy, case 
...
\begin{tabular*}{\tblwidth}{lp{0.05\textwidth}p{0.05\textwidth}p{0.05\textwidth}}
\toprule
\textit{Variable}& Base & High-ash & High-moisture \\ % Table header row
\midrule
Mean boiler efficiency [\%]& 88.2 & 76.6 & 75.9\\
Inlet flue-gas temperature [$K$]& 1352 & 1368 & 1311\\
\midrule
